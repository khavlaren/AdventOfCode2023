...
def day_11_01(lines: list[str]) -> int:
    ...
    line_costs = costs.min(axis=1, initial=1)
    column_costs = costs.min(axis=0, initial=1)
    distances = np.zeros(shape=(len(universes), len(universes)), dtype='float32')
